...
<div class="col-lg-9">
    <div class="table--responsive--md">
        <table class="table" id="dataTable">
            ...
            <tbody>
            @foreach( \UserAuth::totalConfirmedTransfer() as $balance)
                <tr>
            ...
            @endforeach
            @foreach( \UserAuth::gameHistory() as $balance)
                <tr>
                    <td class="trx-type" data-label="Transection Type">{{ ucwords($balance->balance_type) }}</td>
                    <td class="trx-method" data-label="Transection Method">In-game</td>
                    <td class="amount" data-label="Transfer Number">{{ ucwords($balance->game_name) }}</td>
                    <td class="trx-id" data-label="Transection ID"> ------ </td>
                    <td class="" data-label="Amount"> ------ </td>
                    <td class="amount" data-label="Uc-Amount">{{ $balance->balance_amount }}<i class='la la-bitcoin'></i></td>
                    @if( $balance->status == 0 )
                        <td class="status" data-label="status"> <button class="btn btn--primary btn--sm">Losed</button> </td>
                    @elseif( $balance->status == 1 )
                        <td class="status" data-label="status"> <button class="btn btn--success btn--sm">Won</button> </td>
                    @else
                        <td class="status" data-label="status"> <button class="btn btn--danger btn--sm">Rejected</button> </td>
                    @endif
                    <td class="date" data-label="Date">{{ ucwords($balance->created_at) }}</td>
